...
/**
 * Turns the email from a login request into a principal.
 *
...
public class CustomerDetailsService implements UserDetailsService {
    ...
    public SecurityUser loadUserByUsername(String username) throws UsernameNotFoundException {
        return customerRepository.findByEmail(CustomerService.normaliseEmail(username))
                // Mapped here rather than by a SecurityUser constructor taking a Customer, as it was
                // in Phase 8. SecurityUser lives in shared-kernel now because every service needs the
                // principal; the Customer entity lives only here. A library that knew about this
                // service's entity would be a library five services could not use.
                .map(customer -> new SecurityUser(
                        customer.getId(),
                        customer.getEmail(),
